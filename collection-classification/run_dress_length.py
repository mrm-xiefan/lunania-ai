...
from app.models.dress_length import (DressLengthModelContainer,
                                     DressLengthModelContainerAtResNet)
from app.models.manager import ModelStore
from app.prediction.processor import ClassificationProcessor
from app.services.dress_length import DressLengthClassificationService
...
def get_service(model_store: ModelStore, dataset_store: DatasetStore):
    return DressLengthClassificationService(model_store, dataset_store)
...
def command_learn(train_dataset_key: str=None,
    ...
    model_container = DressLengthModelContainerAtResNet(class_num)
...
def command_predict(model_key: str=None, org_dataset_path: str=None,
    ...
    service = DressLengthClassificationService(model_store, dataset_store)

    label = ['above_the_knee', 'below_the_knee', 'cropped']

    processor = ClassificationProcessor(label)

    if not os.path.exists(csv_dir_path):
        os.mkdir(csv_dir_path)
    csv_name = 'dress_length-predict' + dt.now().strftime('-%Y%m%d%H%M%S') + '.csv'
