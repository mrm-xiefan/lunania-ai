...
from app.models.material import (MaterialModelContainer,
                                 MaterialModelContainerAtResNet)
from app.models.manager import ModelStore
from app.prediction.processor import ClassificationProcessor
from app.services.material import MaterialClassificationService
...
def get_service(model_store: ModelStore, dataset_store: DatasetStore):
    return MaterialClassificationService(model_store, dataset_store)
...
def command_learn(train_dataset_key: str=None,
    ...
    model_container = MaterialModelContainerAtResNet(class_num)
...
def command_predict(model_key: str=None, org_dataset_path: str=None,
    ...
    service = MaterialClassificationService(model_store, dataset_store)

    label = ['denim', 'lace', 'leather', 'mesh', 'tulle']

    processor = ClassificationProcessor(label)

    if not os.path.exists(csv_dir_path):
        os.mkdir(csv_dir_path)
    csv_name = 'material-predict' + dt.now().strftime('-%Y%m%d%H%M%S') + '.csv'
