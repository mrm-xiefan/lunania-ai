...
def _shortcut(input, residual):
    ...
    stride_width = input._keras_shape[
        ROW_AXIS] // residual._keras_shape[ROW_AXIS]
    stride_height = input._keras_shape[
        COL_AXIS] // residual._keras_shape[COL_AXIS]
    equal_channels = residual._keras_shape[
        CHANNEL_AXIS] == input._keras_shape[CHANNEL_AXIS]

    if input._keras_shape[
            ROW_AXIS] % residual._keras_shape[ROW_AXIS] != 0:
        stride_width += 1

    if input._keras_shape[
            COL_AXIS] % residual._keras_shape[COL_AXIS] != 0:
        stride_height += 1

    LOGGER.debug('input keras shape:%s',  input._keras_shape)
    LOGGER.debug('residual shape:%s',  residual._keras_shape)
    shortcut = input
    # 1 X 1 conv if shape is different. Else identity.
    if stride_width > 1 or stride_height > 1 or not equal_channels:
        LOGGER.debug('different.')
        LOGGER.debug('subsample:%s', (stride_width, stride_height))
...
def basic_block(nb_filter, init_subsample=(1, 1), is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            ...
            conv1 = _bn_relu_conv(
                nb_filter=nb_filter, nb_row=3, nb_col=3, subsample=init_subsample)(input)

        residual = _bn_relu_conv(
            nb_filter=nb_filter, nb_row=3, nb_col=3)(conv1)
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            ...
            conv_1_1 = _bn_relu_conv(
                nb_filter=nb_filter, nb_row=1, nb_col=1, subsample=init_subsample)(input)

        conv_3_3 = _bn_relu_conv(
            nb_filter=nb_filter, nb_row=3, nb_col=3)(conv_1_1)
        residual = _bn_relu_conv(
            nb_filter=nb_filter * 4, nb_row=1, nb_col=1)(conv_3_3)
        return _shortcut(input, residual)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        conv1 = _conv_bn_relu(nb_filter=64, nb_row=7,
                              nb_col=7, subsample=(2, 2))(input)
        pool1 = MaxPooling2D(pool_size=(3, 3), strides=(
            2, 2), border_mode="same")(conv1)

        block = pool1
        nb_filter = 64
        for i, r in enumerate(repetitions):
            block = _residual_block(
                block_fn, nb_filter=nb_filter, repetitions=r, is_first_layer=(i == 0))(block)
            nb_filter *= 2
