...
from app.models.detail import (DetailModelContainer,
                               DetailModelContainerAtResNet)
from app.models.manager import ModelStore
from app.prediction.processor import ClassificationProcessor
from app.services.detail import DetailClassificationService
...
def define_arg_parser() -> argparse.ArgumentParser:
    ...
    # copy_crop parser
    copy_crop_parser = subparser.add_parser('copy_crop', help='copy crop help')
    copy_crop_parser.add_argument('crop_dataset_path', type=str,
                                  help='cropped image data directory path')
    copy_crop_parser.add_argument('save_dataset_path', type=str,
                                  help='save copied data directory path')

    copy_crop_parser.set_defaults(func=command_copy_crop)
...
def get_service(model_store: ModelStore, dataset_store: DatasetStore):
    return DetailClassificationService(model_store, dataset_store)
...
def command_learn(train_dataset_key: str=None,
    ...
    model_container = DetailModelContainerAtResNet(class_num)
...
def command_predict(model_key: str=None, org_dataset_path: str=None,
    ...
    service = DetailClassificationService(model_store, dataset_store)

    label = ['bijou', 'cutout', 'drawstring', 'embroidery', 'frill',
             'fringe', 'gather', 'high_necked', 'piping', 'pleats',
             'ribbon', 'roll_up', 'sequins', 'slit', 'waist_mark', 'zip_up']

    processor = ClassificationProcessor(label)

    if not os.path.exists(csv_dir_path):
        os.mkdir(csv_dir_path)
    csv_name = 'detail-predict' + dt.now().strftime('-%Y%m%d%H%M%S') + '.csv'
...
def command_copy_crop(crop_dataset_path: str=None, save_dataset_path: str=None, **args):
    image_utils.copy_from_crop_dir(crop_dataset_path, save_dataset_path)
