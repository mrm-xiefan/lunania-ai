...
TOP_MODEL_WEIGHTS_PATH = 'h5py/top_model_color_02.h5'
...
def save_bottlebeck_features():
    ...
    np.save(open('h5py/bottleneck_features_color_02.npy', 'wb'), bottleneck_features)
    
    LOGGER.debug('save features.')

def train_top_model():
    input_data = np.load(open('h5py/bottleneck_features_color_02.npy', 'rb'))
    input_labels = np.array([0] * int((NB_SAMPLES / 2)) + [1] * int((NB_SAMPLES / 2)))
    input_labels = np_utils.to_categorical(input_labels, 2)
    print(input_labels)
    ...
    open(os.path.join('model/','cnn_model_color_02.json'), 'w').write(json_string)
    yaml_string = model.to_yaml()
    open(os.path.join('model/','cnn_model_color_02.yaml'), 'w').write(yaml_string)
